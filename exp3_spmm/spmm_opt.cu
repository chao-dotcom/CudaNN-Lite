    ...
    float_t* __restrict__ d_C)
{
    ...
    // Get row bounds from CSR structure
    int start = d_row_ptr[row];
    int end = d_row_ptr[row + 1];

    // Loop over columns j assigned to this lane (strided by warp size)
    for (int j = lane; j < N; j += 32) {

        float_t sum = 0.0f;

        // Loop over all nonzeros in this row
        for (int p = start; p < end; ++p) {
            int k = d_col_idx[p];
            float_t v = d_vals[p];
            // Accumulate contribution from A[row, k] * B[k, j]
            sum += v * d_B[k * N + j];
        }
...
int main() {
    ...
    // Launch optimized warp kernel
    // Each warp processes one row, so we need M warps total
    // Each warp has 32 threads, so total threads = M * 32
    int block = 256; // 256 threads per block (8 warps per block)
